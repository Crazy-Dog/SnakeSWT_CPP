...
class ScalaSorter {
  ...
  def sort(sortMode: Int) = {
    ...
    sortedList.foreach(file => {
      ...
      var tmpScore = FileStream.getReadedScoreForRewrite();
    ...
    tmpScore = FileStream.getReadedScoreForRewrite();
    FileStream.writeArrayInFile(tmp, new File(pathSorted + "worst_game.out"), tmpScore);
  }

  def compareByScore(o1: File, o2: File): Boolean = {
    //read first file
    var score1 = FileStream.getReadedScore(o1);

    //read second file
    var score2 = FileStream.getReadedScore(o2);
    if (score1 > score2)
      return true;
    return false;
  }
