...
public class Game extends Composite {
  ...
  private void drawObjects(Event event) {
    ...
    graphicClass.drawImage(cherry, cherry_x, cherry_y); // draw cherry

    // draw snake
    for (int snake_part = 0; snake_part < dots; snake_part++) {
      if (snake_part == 0) {
        graphicClass.drawImage(head, snake_x[snake_part], snake_y[snake_part]);
      } else {
        graphicClass.drawImage(ball, snake_x[snake_part], snake_y[snake_part]);
      }
    }
  }
  ...
  private void move() {
    try {
      ...
      // check scope
      if (level > 5 || (snake_y[0] > size.y - DOT_SIZE) || snake_y[0] < 0
          || (snake_x[0] > size.x - DOT_SIZE) || snake_x[0] < 0)
        throw new IOException();
